...
> **当前版本：v1.2.5**（v1.2 分组/文件夹；v1.2.1 组头播放按钮；v1.2.2 分P断点续播；v1.2.3 分P结束误判；v1.2.4 断点/连播系统改造；v1.2.5 修复「已看完(done)被进度快照抹掉」「同步历史 viewAt 报错」并新增绿色【正在看】标识；版本变更须同步更新 manifest.json 与本文档）。
...
- [x] （v1.2.5）**修复+新功能**：①修复「已看完(done)」被随后的进度快照写入（tick/pagehide/capture/sync 为整体替换）抹掉——改为合并写入保留 done，`onEnded` 完成时 time 记到视频末尾；②修复 `syncHistory` 因 `viewAt` 变量缺失抛 ReferenceError；③新增绿色【正在看】标识——每组（含未分组整体）按「未看完里最近更新，否则第一个未看完；组内从未有进度则无；全部看完留在最后那条」算一个，纯派生无新增存储，进度轻量更新时自动递进。

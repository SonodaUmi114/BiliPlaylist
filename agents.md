...
- **侧边栏交互**：
  ...
  - 侧边栏头部有**全局刷新按钮**（刷新列表 + 同步观看历史，如跨页面添加视频后点刷新即可看到最新列表）
...
- **播放进度记忆（B 站播放器恢复 + 插件捕获保存）**：**插件不主动 seek**——打开视频时由 B 站播放器自身恢复到"上次看到"（官方最新，播放器会显示该文案），避免插件旧值覆盖；插件只负责**捕获保存**：① 页面加载 5s 后读取播放器实际位置 ② 关闭标签页/切走时保存一次精确位置 ③ **打开视频页/空间页自动同步官方历史（节流 1 分钟一次）**④ 全局刷新按钮手动同步（≤5 页，找到即停）；仅当 B 站未恢复断点（5s 后仍在开头）时插件才本地兜底 seek；只保存播放列表内视频（视频名/UP名/分P/断点永久保留，不受 B 站历史清理影响）；旧 video 轮询方案停用（备份分支 `backup/old-progress-video-element`）
...
- [x] 侧边栏条目信息：视频名称、UP主名、分P进度（如 `1/11`，只显示总数）、当前分P播放时间 `hh:mm:ss`、**播放按钮 ▶（新标签页打开）**、删除按钮、已播完标记；**全局刷新按钮**在侧边栏头部 = 重新同步列表 + 同步播放进度
...
- [x] 播放进度记忆：需要（含分P位置）；**B 站播放器自身恢复"上次看到"（插件不主动 seek，避免覆盖），插件只捕获保存**（加载 5s 后位置 + 关闭时一次 + 刷新按钮合并历史同步 ≤5 页），本地永久保存播放列表内视频历史；旧 video 轮询方案停用（备份分支 `backup/old-progress-video-element`）

...
- **播放进度记忆（官方历史方案）**：断点来源为 **B 站官方观看历史接口**——侧边栏「历史」按钮 / 视频页自动同步，只保存**播放列表内**视频到本地（视频名/UP名/分P/断点永久保留，不受 B 站历史清理影响），打开视频时恢复断点；旧 video 元素轮询方案已停用（备份分支 `backup/old-progress-video-element`）

...
- [x] （v1.2，已实现）侧边栏**分组/文件夹**：成组（按钮「成组」，允许重新成组）+ 文件夹标签（浅色色块可改色 / 重命名 / 收起展开，默认收起，收起显示成员数与首条摘要）+ 自动排序**组内排、组间不排**（点组头自动全选组内成员）+ 拖进/移出、拖标签改位、取消分组。详细规格见 `docs/features-group.md`

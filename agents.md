...
- **播放进度记忆（官方 + 本地兜底）**：断点来源 = ① 打开视频时定向捕获页面 `__INITIAL_STATE__.progress`（官方上次进度）② ⟲ 按钮小量历史翻页（≤5 页，找到即停）③ **关闭标签页/切走时本地保存一次精确位置**（`video.currentTime`，解决官方上报滞后）；恢复时本地/官方取较新值，**每次从播放列表打开都是最新进度**；只保存播放列表内视频，本地永久保留；旧 video 轮询方案停用（备份分支 `backup/old-progress-video-element`）
...
- [x] 播放进度记忆：需要（含分P位置）；**断点来源改为官方观看历史接口**（本地永久保存播放列表内视频历史），旧 video 元素轮询方案停用（备份分支 `backup/old-progress-video-element`）

...
> **当前版本：v1.2.0**（在 v1.1 基础上新增「侧边栏分组/文件夹」；版本变更须同步更新 manifest.json 与本文档）。
...
- [x] （v1.2，已实现）侧边栏**分组/文件夹**：成组（按钮「成组」，允许重新成组）+ 文件夹标签（选中勾选指示 / 重命名 / 收起展开，默认收起，收起显示成员数与首条摘要）+ 自动排序**组内排、组间不排**（点组头自动全选组内成员）+ 拖进/移出、拖标签改位、取消分组（**无颜色功能**）。详细规格见 `docs/features-group.md`

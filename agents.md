...
- **侧边栏交互**：
  ...
  - 右下角**热区按钮簇**平时隐藏，鼠标移动到右下角**热区**（120px）附近时才显示，位于热区中心（距边约 60px）；点击**侧边栏按钮**（☰）打开/收起侧边栏
  - 同一热区在侧边栏按钮**左侧平行**显示页面相关按钮：视频页为**「＋ 加入列表」**、空间页为**「多选」**（两者页面不同，不冲突）
  - 每个视频项显示：**视频名称、UP主名、分P进度（如 `3/11`，无"分P"字样）、当前分P播放时间 `hh:mm:ss`、播放按钮（▶，点击播放）、删除按钮**；点击条目本身不跳转；**已播放完的视频显示完成标记**
  - 侧边栏头部有**全局刷新按钮**（重新从存储同步列表 + 同步当前播放进度，如跨页面添加视频后点刷新即可看到最新列表）
  - **多选默认关闭**：点热区「多选」按钮开启，才显示【已选 N / 加入播放列表 / 清空】工具栏；**整卡覆盖层**为点击目标（多选模式点卡片即切换选中，不进入播放页），小复选框绝对定位在**日期行高度、卡片右端**作选中指示，并抑制缩略图悬停预览
...
- [x] 侧边栏条目信息：视频名称、UP主名、分P进度（如 `3/11`）、当前分P播放时间 `hh:mm:ss`、**播放按钮 ▶**（点击播放，条目本身不跳转）、删除按钮、已播完标记；**全局刷新按钮**在侧边栏头部 = 重新同步列表 + 同步播放进度
- [x] 多选：**默认关闭**，热区「多选」按钮开启；**整卡覆盖层点击切换选中**（不进入播放页，抑制缩略图悬停预览），小复选框绝对定位在日期行高度、卡片右端作指示

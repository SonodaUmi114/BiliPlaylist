...
- **侧边栏交互**：
  ...
  - 每个视频项显示：**视频名称、UP主名、分P进度（如 `3/11`，无"分P"字样）、当前分P播放时间 `hh:mm:ss`、删除按钮**；**已播放完的视频显示完成标记**
  - 侧边栏头部有**全局刷新按钮**（重新从存储同步列表 + 同步当前播放进度，如跨页面添加视频后点刷新即可看到最新列表）
  - **多选默认关闭**：点热区「多选」按钮开启，才显示【已选 N / 加入播放列表 / 清空】工具栏与卡片复选框（复选框位于视频名称下方**日期行最右端**）
...
- [x] 侧边栏：仅 B 站站内生效；右下角**热区按钮簇**（平时隐藏、热区悬停显示，位于热区中心）：侧边栏按钮 + 页面按钮（视频页「＋ 加入列表」/ 空间页「多选」）左侧平行
- [x] 侧边栏条目信息：视频名称、UP主名、分P进度（如 `3/11`）、当前分P播放时间 `hh:mm:ss`、删除按钮、已播完标记；**全局刷新按钮**在侧边栏头部 = 重新同步列表 + 同步播放进度
- [x] 多选：**默认关闭**，热区「多选」按钮开启；复选框位于视频名称下方日期行最右端

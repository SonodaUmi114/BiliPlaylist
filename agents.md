...
- **播放进度记忆（B 站播放器恢复 + 插件捕获保存）**：**插件不主动 seek**——打开视频时由 B 站播放器自身恢复到"上次看到"（官方最新，播放器会显示该文案），避免插件旧值覆盖；插件只负责**捕获保存**：① 页面加载 5s 后读取播放器实际位置 ② 关闭标签页/切走时保存一次精确位置 ③ 全局刷新按钮合并小量历史翻页（≤5 页，找到即停）；仅当 B 站未恢复断点（5s 后仍在开头）时插件才本地兜底 seek；只保存播放列表内视频（视频名/UP名/分P/断点永久保留，不受 B 站历史清理影响）；旧 video 轮询方案停用（备份分支 `backup/old-progress-video-element`）

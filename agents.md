...
- **播放进度记忆（官方方案）**：断点主来源为**视频页 `__INITIAL_STATE__` 定向捕获**（B 站续播用的官方数据，打开播放列表视频即得，零历史翻页）；侧边栏「⟲」按钮做小量历史翻页补充（≤5 页，找到即停）；只保存**播放列表内**视频到本地（视频名/UP名/分P/断点永久保留，不受 B 站历史清理影响），打开视频时恢复断点；旧 video 元素轮询方案已停用（备份分支 `backup/old-progress-video-element`）
